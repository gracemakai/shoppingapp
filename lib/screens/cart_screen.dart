...
class CartScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        //Get total products selected to buy
        int total = 0;
        for (var i = 0; i < _shopController.toBuyProducts.length; i++) {
          total = total + _shopController.toBuyProducts.elementAt(i).price!;
        }
                    ...
                    InkWell(
                      onTap: () {
                        if (_shopController.toBuyProducts.isNotEmpty) {
                          for (var i = 0;
                              i < _shopController.toBuyProducts.length;
                              i++) {
                            _shopController.checkoutProducts.removeWhere(
                                (element) =>
                                    element.id ==
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(10),
                            color: _shopController.toBuyProducts.isNotEmpty
                                ? Colors.green
                                : Colors.grey,
                            boxShadow: [
                              BoxShadow(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ProductModel product =
            _shopController.checkoutProducts.elementAt(index);
        return Padding(
          ...
          child: Row(
            ...
            children: [
              Obx(() {
                return Radio(
                    value: _shopController.toBuyProducts.indexWhere(
                            (element) => element.id == product.id) ==
                        -1,
                    toggleable: true,
                    groupValue: false,
                    onChanged: (value) {
                      if (_shopController.toBuyProducts.indexWhere(
                              (element) => element.id == product.id) ==
                          -1) {
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          Align(
                            child: InkWell(
                              onTap: () {
                                if (_shopController.checkoutProducts.indexWhere(
                                        (element) =>
                                ...
                                _shopController.checkoutProducts.refresh();
                                GetSnackBar(
